...
class NLPCleanScraper:
    ...
    def crawl_guokr(self, total_target):
        print(f"\n[开始爬取果壳网科普] 目标: {total_target}条清洗后的语料")
        # 果壳网科学人栏目
        ...
        while count < total_target and queue:
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    # 1. 尝试提取文章内容
                    # 果壳文章页通常包含 JSON 数据在 script 标签中
                    # 或者直接提取大段文本
                    if '/article/' in url:
                        # 提取正文：简单策略，提取所有 p 标签文本，过滤短的
                        ...
                        if self.is_valid(cleaned):
                            # 再次检查长度，确保是长文
                            if len(cleaned) > 100:
                                self._save_line(cleaned, "corpus_kepu_clean.txt")
                                count += 1
                                print(f"果壳抓取进度: {count}/{total_target} - {url}")
                    ...
                    for link in soup.find_all('a', href=True):
                        ...
                        # 补全链接
                        if href.startswith('/'):
                            full_url = f"https://www.guokr.com{href}"
                        ...
                        # 只追踪果壳站内的文章或列表
                        if 'guokr.com' in full_url and ('/article/' in full_url or '/scientific/' in full_url):
