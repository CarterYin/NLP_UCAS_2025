...
3.  **模型规模影响**：对比分析不同参数量（7B, 14B, 72B）的模型对分词性能的影响。
...
#figure(
  table(
    ...
    [7B-Instruct], [0.6569], [0.7755], [0.7113],
    [14B-Instruct], [0.2532], [0.2584], [0.2557],
    [72B-Instruct], [0.6815], [0.8009], [0.7364],
  ),
  caption: [不同参数量模型的性能对比],
)

**分析**：
- **72B模型**表现最佳（F1=0.7364），证明了更大参数量带来的语义理解优势。
- **14B模型**表现异常低（F1=0.2557）。经检查，该模型在某些情况下未能严格遵循“用空格分隔”的指令，或者输出了包含解释性文字的内容，导致格式解析失败。这表明中等规模模型在指令遵循的稳定性上可能存在波动。
